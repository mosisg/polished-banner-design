// Constants and utility functions
const STRAPI_URL = import.meta.env.VITE_STRAPI_URL || "https://your-strapi-api.com"; // Update to default to your hosted Strapi URL
const STRAPI_API_KEY = import.meta.env.VITE_STRAPI_API_KEY || "";

// Enable mock data as fallback when the API is unavailable
export const USE_MOCK_DATA = true;

// Allowed origins for CORS - updated to include all possible Lovable domains
export const ALLOWED_ORIGINS = [
  'http://localhost:8080',
  'http://localhost:1337',
  'https://your-strapi-api.com', // Add your hosted Strapi domain
  'https://preview--polished-banner-design.lovable.app',
  'https://polished-banner-design.lovable.app',
  'https://lovable.app',
  '*.lovable.app', // Wildcard for all Lovable subdomains
  window.location.origin // Always include current origin
];

export const getStrapiURL = (): string => {
  return STRAPI_URL;
};

export const getStrapiAPIKey = (): string => {
  return STRAPI_API_KEY;
};

export const getStrapiMedia = (url: string | null | undefined): string => {
  if (!url) return "";
  
  if (url.startsWith("http") || url.startsWith("//")) {
    return url;
  }
  
  if (url.startsWith("/")) {
    // Likely a placeholder or local asset
    return url;
  }
  
  // Return clean URL without cache buster to prevent issues with media
  return `${STRAPI_URL}${url}`;
};
